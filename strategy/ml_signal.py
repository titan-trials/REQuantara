...
def run_ml_strategy(ticker, start, end, initial_capital, stop_loss, position_size, df=None):
    ...
    print(f"Test Up-Day Rate   : {metrics['Up_Day_Rate']}%  (days risen, not a win rate)")
    print(f"Buy & Hold Return  : {metrics['Market_Return']}%")
    print(f"VS BUY & HOLD      : {metrics['Vs_Market']:+.2f} pts")
