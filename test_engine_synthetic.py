...
df = run_backtest(frame([100, 120, 150, 200], [1, 1, 1, 1]), 10000, 0.05, 1.0)
...
df = run_backtest(frame([100, 120, 150, 200], [1, 1, 1, 1]), 10000, 0.05, 0.5)
...
df = run_backtest(frame([100, 96, 94, 80], [1, 1, 1, 1]), 10000, 0.05, 1.0)
...
df = run_backtest(frame([100, 94, 70, 60], [1, 1, 1, 1]), 10000, 0.05, 1.0)
...
df = run_backtest(frame([100, 110, 130, 130], [1, 1, 0, 0]), 10000, 0.05, 1.0)
...
for ps in (0.02, 0.05, 0.20, 0.50, 1.0):
    a = run_backtest(frame(cheap, [1, 1, 1, 1]), 10000, 0.05, ps)
    b = run_backtest(frame(pricey, [1, 1, 1, 1]), 10000, 0.05, ps)
...
for ps in (0.02, 0.05, 0.20, 0.50, 1.0):
    df = run_backtest(frame([365.0, 400.0, 500.0, 547.5], [1, 1, 1, 1]), 10000, 0.05, ps)
...
df = run_backtest(frame([100.0, 200.0], [1, 1]), 10.0, 0.05, 0.05)  # $0.50 notional
